#!/usr/bin/env python3
"""
Standard station — floor / interior piece.
...
Optional (INTERIOR_DETAILS=True):
  - Waiting room: 2 rows of dual (back-to-back) benches centred in room (1905), stove stub
  ...
  - Freight room: crates, barrels, bags (more items for wider room), stove stub, standup desk
...
Output:
  FCStd: CADtrains/Station/freecad/Std_FloorBasic.FCStd
  STL:   CADtrains/Station/printed_files/Std_FloorBasic.stl
"""
...
BLDG_D    = ft(15,  0)   # 52.6mm  (SK was 40.0mm)
...
# ---- Ticket wicket ----------------------------------------------------------
WICKET_W = 7.0
WICKET_Z = ft(4, 0)                       # 14.0mm
WICKET_H = FLOOR_T + INT_DOOR_H - WICKET_Z  # top at door opening top
...
print(f"Std Floor  BLDG_L={BLDG_L:.2f}  BLDG_D={BLDG_D:.2f}  WALL_H={WALL_H:.2f}")
...
for sign, cx in [(-1, T_CX - (HW + FHW)/2), (1, T_CX + (HW + FHW)/2)]:
    ...
    lp = [V(cx + sign*hww,              cy - hww,            WIN_SILL),
          V(cx - sign*hww,              cy + hww,            WIN_SILL),
          V(cx - sign*(hww + WT/SQ2),   cy + hww - WT/SQ2,  WIN_SILL),
          V(cx + sign*(hww - WT/SQ2),   y_D,                 WIN_SILL),
          V(cx + sign*hww,              cy - hww,            WIN_SILL)]
    floor = floor.cut(Part.Face(Part.makePolygon(lp)).extrude(V(0, 0, WIN_H_SIDE)))

# Center bay window frame
...
floor = floor.fuse(Part.makeBox(CENTER_WIN_W + 2*FRAME_W, FRAME_D, FRAME_W,
                                 V(cx0 - FRAME_W, bay_inner_y - FRAME_D, z_head)))
floor = floor.fuse(Part.makeBox(CENTER_WIN_W + 2*FRAME_W, FRAME_D, FRAME_W,
                                 V(cx0 - FRAME_W, bay_inner_y - FRAME_D, z_sill)))

# Side bay window frames
for sign, cx in [(-1, T_CX - (HW + FHW)/2), (1, T_CX + (HW + FHW)/2)]:
    cy   = BAY_Y0 + BAY_PROJ / 2
    y_D  = max(cy - hww - WT/SQ2, BAY_Y0 + 0.2)
    Dx = cx + sign * (hww - WT/SQ2);  Dy = y_D
    Cx = cx - sign * (hww + WT/SQ2);  Cy = cy + hww - WT/SQ2
    pdx = -sign / SQ2;  pdy = 1.0 / SQ2
    ndx = -sign / SQ2;  ndy = -1.0 / SQ2
    fw = FRAME_W
    fd = FRAME_D
    z_sf = WIN_SILL - fw
...
if INTERIOR_DETAILS:
    ...
    # ---- WAITING ROOM -------------------------------------------------------
    # Two rows of dual (back-to-back) benches, centred in room — 1905 practice.
    # Each unit: two 18" seats sharing a 4" central back, 6' long, running in X.
    B_SEAT_D = ft(1, 6)                    # 18" seat depth per side ≈ 5.25mm
    B_BACK_T = ft(0, 4)                    # 4" back thickness ≈ 1.15mm
    B_BACK_H = ft(1, 6)                    # 18" back height above seat ≈ 5.25mm
    B_SEAT_H = ft(1, 6)                    # 18" seat height ≈ 5.25mm
    B_LEN    = ft(6, 0)                    # 6' bench length ≈ 21mm (runs in X)
    B_DEPTH  = 2 * B_SEAT_D + B_BACK_T    # total bench unit depth ≈ 11.55mm
    B_AISLE  = ft(3, 0)                    # 36" centre aisle ≈ 10.5mm
    B_ROW_OFF = B_DEPTH / 2 + B_AISLE / 2 # row centre offset from room Y centre

    b_x0 = W_CX - B_LEN / 2              # bench centred on waiting room X
    for row_yc in [door_cy - B_ROW_OFF, door_cy + B_ROW_OFF]:
        b_y0 = row_yc - B_DEPTH / 2
        # Seat base (full bench depth at seat height)
        floor = floor.fuse(Part.makeBox(B_LEN, B_DEPTH, B_SEAT_H,
                                         V(b_x0, b_y0, FLOOR_T)))
        # Central back divider (above seat, at Y centre of unit)
        floor = floor.fuse(Part.makeBox(B_LEN, B_BACK_T, B_BACK_H,
                                         V(b_x0, b_y0 + B_SEAT_D, FLOOR_T + B_SEAT_H)))
    floor = add_stove_with_guard(floor, X_WT_WALL - STOVE_S - 0.5, WALL_T + 0.5)
    print(f"  Waiting room: 2×dual benches centred (B_DEPTH={B_DEPTH:.2f} B_LEN={B_LEN:.2f}"
          f" rows Y={door_cy-B_ROW_OFF:.1f},{door_cy+B_ROW_OFF:.1f}) + stove stub (WT wall)")
    ...
    # Center + mid-room scatter — more items for the wider freight room
    for sc_x, sc_y in [(F_CX-16, 18), (F_CX-12, 22), (F_CX-8, 16),
                       (F_CX-4,  22), (F_CX,    16), (F_CX+5, 21),
                       (F_CX+10, 19), (F_CX+14, 23)]:
        floor = floor.fuse(Part.makeBox(cS, cS, cS, V(sc_x, sc_y, FLOOR_T)))
    for bg_x, bg_y in [(F_CX-10, BLDG_D*0.6), (F_CX-5, BLDG_D*0.55),
                       (F_CX+2, BLDG_D*0.5), (F_CX+6, BLDG_D*0.65)]:
    ...
    # Standup desk — TF wall, beside notice board
    sd = ft(1, 0)
    ...
    print("  Freight room: corner1(2×large+small stack), corner2(large-B+2 barrels),"
          " expanded center scatter, stove stub (TF wall), standup desk")
...
doc_name = f"Std_FloorBasic{vtag}"
