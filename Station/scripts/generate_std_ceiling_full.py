#!/usr/bin/env python3
"""
Standard station — full-building ceiling (waiting + ticket office + bay + freight).
...
Coordinate system matches other standard station scripts (building coordinates):
...
Slab footprint: ~211mm X × 100mm Y — fits 250×210mm build plate.

Output:
  FCStd: CADtrains/Station/freecad/Std_CeilingFull.FCStd
  STL:   CADtrains/Station/printed_files/Std_CeilingFull.stl
"""
...
TICKET_W  = ft(10,  2)   # 35.6mm  (same as SK)
...
F_CX = X_TF_WALL + WALL_T + FREIGHT_W / 2
...
print(f"Std Ceiling Full  BLDG_L={BLDG_L:.2f}  BLDG_D={BLDG_D:.2f}  GRID_P={GRID_P}mm")
...
print(f"  Slab: {x1-x0:.2f} x {y1-y0:.2f}mm")
...
for label, ex in [("waiting-end", 0.0), ("freight-end", BLDG_L)]:
    ...
    print(f"  Channel: {label}  X={cx-CHAN_W/2:.1f}..{cx+CHAN_W/2:.1f}  Y=0..{BLDG_D:.2f}")
...
# Passenger wall — split around bay opening
...
print(f"  Channel: passenger wall  left X=0..{left_end:.1f}  right X={right_sta:.1f}..{BLDG_L:.2f}")
...
while gx <= gx1 + GRID_P:
    if bx0 <= gx <= bx1:
        if gx <= T_CX - FHW:
            y_end = min(BLDG_D + (gx - bx0), by1)
        elif gx >= T_CX + FHW:
            y_end = min(BLDG_D + (bx1 - gx), by1)
        else:
            y_end = by1
    else:
        y_end = gy1
...
# =============================================================================
# 6. RAFTER TAIL TICK MARKS on eave soffits
# =============================================================================
TICK_R     = 0.5
TICK_D     = 0.3
TICK_SPACE = 14.0
...
def cut_tick_row(shape, positions, fixed_coord, axis, z):
    for p in positions:
        cx, cy = (p, fixed_coord) if axis == 'x' else (fixed_coord, p)
...
tick_cnt += len(x_ticks)
print(f"  Ticks: passenger eave  {len(x_ticks)} marks  Y={BLDG_D+EAVE_SIDE/2:.2f}")
...
tick_cnt += len(y_ticks)
print(f"  Ticks: freight gable eave  {len(y_ticks)} marks  X={BLDG_L+EAVE_END/2:.2f}")
...
# EXPORT
# =============================================================================
fc_path = "/home/abyrne/Projects/Trains/CADtrains/Station/freecad/Std_CeilingFull.FCStd"
try: FreeCAD.closeDocument("Std_CeilingFull")
except: pass
doc = FreeCAD.newDocument("Std_CeilingFull")
...
stl = "/home/abyrne/Projects/Trains/CADtrains/Station/printed_files/Std_CeilingFull.stl"
